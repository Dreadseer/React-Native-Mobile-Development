...
| POST | `/api/order/{id}/status` | Bearer | Update order status — body: `{ "status": "in progress" }` (lowercase string) |
| GET | `/api/order-statuses` | Bearer | ~~Fetch status ID mapping~~ — **NOT NEEDED**, status update uses name string directly |
...
- **Status update body is `{ status: "in progress" }`** — lowercase string value, field name is `status`. NOT `{ order_status_id: number }`. Sending `order_status_id` returns `400 Bad Request: "Status is required"`.
- **`GET /api/order-statuses` is NOT needed** — the status update endpoint accepts the status name string directly. Do not build a statusMap from IDs.
- **Order status strings from API are lowercase** — `"pending"`, `"in progress"`, `"delivered"`. Normalize to uppercase on fetch for internal use, convert back to lowercase when sending to the API.
...
### Confirmed Order Field Names (from API logs)
...
| `status_id` | **not present in response** |

### Confirmed Order Types
